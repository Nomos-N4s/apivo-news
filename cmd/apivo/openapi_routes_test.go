...
func TestOpenAPISecurityMatchesTheAuthGate(t *testing.T) {
	...
	for pattern, op := range documentedOperations(t, doc) {
		...
		gated := strings.HasPrefix(path, editorialPrefix) ||
			...
			strings.HasPrefix(path, destinationsPrefix)
		switch {
...
func (unreachableOpsAwaiting) WithdrawalsAwaitingApproval(context.Context, ops.WithdrawalAfter, int) ([]ops.AwaitingWithdrawal, error) {
	return nil, errors.New("unreachable")
}
