...
- [ ] T037 [P] Test that `cashback.provenance` returns the full payout→click chain in one query (C-7) in `internal/cashback/ops/provenance_test.go`
...
- [ ] T072 [P] [US1] Contract tests for `POST /clickouts` (401 unauthenticated, 409 expired offer, 429 rate limited, no click row after deeplink failure) in `internal/cashback/clickout/handlers_test.go` — all but the 429 landed with T064/T065; what is left waits on T066, which is what can return one
