...
- [x] T116 [P] [US6] Integration test: an omitted and a shorted transaction are both flagged with deltas and change no balance, in `internal/cashback/ops/reconciliation_integration_test.go`
...
- [x] T126 Handle `identity.account.deleted` by closing participation and flagging in-flight withdrawals, never deleting financial rows, in `internal/cashback/wallet/account_lifecycle.go`. Flagging is derived, not stored: `withdrawal_request` gains no column and no state, and the flag is `cashback.withdrawal.flagged`. Wiring the registry to drive it made this the stream's first subscriber. **No `GET …/ops/withdrawals` exists**, so the flag has no queue to appear on yet — that list is the follow-up
