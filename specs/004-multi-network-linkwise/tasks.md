...
- [x] T200 [US2] Migration `<n+1>_click_backs_one_credit.{up,down}.sql` in `internal/platform/db/migrations/`: drop `entry_click_id_idx`, recreate it **unique** over `(click_id) where click_id is not null and reversal_of_id is null`, with the index comment stating why reversals are excluded
- [x] T201 [US2] Test in `internal/cashback/earnings/` against real Postgres: two non-reversal credits citing one click through different `network_transaction_id` — assert SQLSTATE `23505` naming `entry_click_id_idx`; and assert a reversal citing the credit's own click is still permitted
- [x] T202 [US2] Teach the crediting path to recognise that refusal **by name**, the way `earnings/open.go:200-201` already recognises `entry_one_per_report`, so a duplicate is an unattributed row rather than a 500
- [x] T203 [US2] Migration `<n+3>_click_carries_its_network.{up,down}.sql` (0037 in the tree): `offer_id_merchant_network_unique`, `merchant_network_id_network_unique`, the two new `cashback.click` columns, the backfill, `set not null`, both composite foreign keys, and `click_network_id_idx`
- [x] T204 [US2] Test against real Postgres: a click whose `network_id` names a network its route does not belong to is refused with SQLSTATE `23503` naming `click_network_matches_route`; and the backfill leaves no null on a seeded database
- [x] T205 [US2] `internal/cashback/clickout/queries/click.sql`: `GetClickByRef` takes the reporting network and returns a row only when `click.network_id` matches; write `network_id` at click-out. Regenerate `internal/cashback/clickout/store` with sqlc
- [x] T206 [US2] Attribution: a reference matching a click on **another** network yields no match and queues the report as unattributed with reason `foreign_network` — with a test that the *sibling* report on the issuing network still credits normally. Landed through its own statement, `RecordForeignNetworkReference`; the reason *column* is T225's, and until it lands which statement wrote the row is the record of why
- [x] T207 [P] [US3] Migration `<n+2>_preferred_route_must_be_publishable.{up,down}.sql`: `merchant_network.can_attribute boolean not null default true`, and `merchant_network_preferred_is_publishable` check, both with comments
- [x] T208 [US3] Test against real Postgres: preferring a `left_network` route and preferring an `active` route whose `can_attribute` is false are each refused with SQLSTATE `23514` naming the constraint (SC-022, SC-026)
- [x] T209 [US3] The demotion path in `internal/cashback/catalogue/`: when a route becomes `paused`/`left_network`, or its network becomes inactive, its preference is withdrawn and a surviving publishable route takes over — recorded, with who and why
- [x] T210 [P] [US3] Operator query: retailers that have publishable routes and publish nothing. This is the cross-row rule [data-model.md](data-model.md) deliberately does not make a constraint, and it must be visible rather than silent

- [x] T254 [US2] **The click-out refuses a member who has not opted in** (FR-110), in `internal/cashback/clickout/`. Nothing checks it today, so a signed-in member who never accepted terms can click out and be credited. This lands **before** T255, or that constraint would refuse a member who had already bought something
- [x] T255 [US2] Migration `<n+4>_entry_currency_is_the_members.{up,down}.sql`: `participation_account_currency_unique`, and `entry_currency_is_the_members` as a composite foreign key from `(account_id, currency)`. Plus `network_account.reports_currency`, nullable, with its format check
- [x] T256 [US2] Test against real Postgres: an entry whose currency differs from the member's participation currency is refused with SQLSTATE `23503` naming the constraint; and a participating member's `default_currency` cannot be restated while an entry references the old one (SC-027)
- [x] T257 [US2] The crediting path recognises that refusal by name and queues the report for an operator with the currency in it (FR-109), rather than failing the whole window
- [x] T258 [US1] `connect-network` and the seed refuse a publisher account whose **declared** `reports_currency` is not the deployment's payout currency, naming both (FR-108). A null is reported, never refused — nobody has established it yet, and that is the state to make visible
...
- [x] T211 [US1] `cmd/apivo/registry.go`: one `map[config.NetworkDriver]registration` carrying each shipped driver's documented facts and its constructor. In the composition root, never in the domain — `internal/arch/network_isolation_test.go` rule A forbids a domain package naming an adapter
- [x] T212 [US1] Delete the `networkAdapter` switch (`cmd/apivo/networks.go:138`) into the registry
- [x] T213 [US1] Delete the `documentedNetwork` switch (`cmd/apivo/connect_network.go:190`) into the registry
- [x] T214 [US1] Test in `cmd/apivo/`: every registry entry is both **servable and seedable**, and a driver absent from the registry is absent from both. This is the defect FR-092 names — a driver that can be seeded into `cashback.network` and then refuses to start
...
- [ ] T228 [US4] `GET /api/v1/cashback/ops/networks`: connected networks, each account's cursors, last poll and outcome, `driver_shipped`, and `credential_present` as a **boolean** — never the credential, never a prefix, never its length (ADR-0003). **Mostly landed (#605, #606): everything here except the last poll and its outcome.** Nothing in this repository stores those — there is no job-run table, `internal/platform/scheduler` persists nothing, and a sweep's result reaches the logs and stops. Recording them means storage plus a write on every sweep and catalogue import, three per account, and is [#607](https://github.com/Nomos-N4s/apivo-news/issues/607). The five causes of silence that need no new writes are served: `driver_shipped`, `credential_present`, both `active` flags, `backfill_from` and `reporting_lag_minutes`
...
- [x] T242 [US1] Record and redact real Linkwise responses into `internal/cashback/networks/linkwise/testdata/`, same standard as T236. **This task is the gate**: it either produces recordings or it reports that it cannot, and nothing below starts on the second outcome — five recordings: transactions in both envelope shapes, an empty window, a structured 400, and ten programmes of the joined list. The publisher identifier is replaced with a placeholder
- [x] T243 [US1] `internal/cashback/networks/linkwise/`: package skeleton — `ID`, `Account`, `Limits` from the recording, its own rate limiter and retry budget (FR-106). Every number measured: a seven-day window as a **latency budget** (cost tracks window width, ~0.2s/day, and there is no paging), and a self-imposed 20/min because Linkwise publishes no limit and sends no rate-limit header
- [x] T244 [US1] Linkwise catalogue: `FetchCatalogue` over `programs.html` — **not** the XML/CSV product feed §5.1 describes, and **not** `rest_programs.html`, which is what third-party source suggested. The real endpoint and its parameters came from its own usage text. It asks `joined=yes&status=all`: the default is `status=active`, under which a paused programme vanishes and an import reads absence as departure
- [x] T245 [US1] Linkwise `can_attribute` **per programme** (rule 11), from the `allow_deeplinking` flag Linkwise publishes on each programme. Where the adapter cannot determine it, it declares the network unattributable rather than defaulting to attributable — carried as `MerchantStatusPaused`, because `ReportedMerchant` has no `can_attribute` field of its own. Three of the 334 joined programmes allow cashback sites AND refuse deeplinking, so reading only the promotion methods would have published all three
- [x] T246 [US1] `linkwise.BuildDeeplink`: the click reference in Linkwise's own parameter, wrapping `ErrDeeplinkNotFormed` on any refusal (rule 5). Assert `IssuedClickRef` survives the round trip — ≥22 URL-safe characters, byte for byte. Also refuses a slot outside `subid1`–`subid5` and an unescaped `lnkurl`. **The slot NAME is still inferred — see #524**
- [x] T247 [US1] `linkwise.FetchTransactions`, every field from T242's recording. **If sale and commission arrive in different currencies, stop**: that is Q13, and `Reported.Validate` refuses it at the port by design — they cannot, because the report carries **no currency field at all**. It is joined from the programme, which is per-programme and not uniform: 329 EUR, 3 PLN, 2 USD
- [x] T248 [US1] Register `linkwise` in the driver registry; the conformance suite runs it green — 17 scenarios, none skipped, driven through a local server answering the adapter's own recordings. Nothing test-only was added to the adapter: a real transport is the knob the fixture needs constructor options for
