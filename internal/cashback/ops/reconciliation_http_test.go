...
func reconcile(t *testing.T, store ops.ReconciliationStore, method, path, body string) *httptest.ResponseRecorder {
	...
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{}, unreachableRefuser{}, unreachableSettler{}, store, unreachableHeld{}, unreachableDestinations{}, stubAuth{op: anOperator}).ServeHTTP(rec, req)
	return rec
}
