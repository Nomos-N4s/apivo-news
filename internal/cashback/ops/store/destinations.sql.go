...
const getDestinationForVerification = `-- name: GetDestinationForVerification :one
select
    d.id,
    d.account_id,
    d.kind,
...
type GetDestinationForVerificationRow struct {
	ID             pgtype.UUID
...
func (q *Queries) GetDestinationForVerification(ctx context.Context, id pgtype.UUID) (GetDestinationForVerificationRow, error) {
	row := q.db.QueryRow(ctx, getDestinationForVerification, id)
	var i GetDestinationForVerificationRow
	err := row.Scan(
		&i.ID,
		&i.AccountID,
		&i.Kind,
...
const listUnverifiedDestinations = `-- name: ListUnverifiedDestinations :many
...
select
 ...
 where d.verified_at is null
   and (d.created_at, d.id) > ($1::timestamptz, $2::uuid)
 order by d.created_at, d.id
 limit $3
`

type ListUnverifiedDestinationsParams struct {
	AfterCreatedAt pgtype.Timestamptz
	AfterID        pgtype.UUID
	PageSize       int32
}

type ListUnverifiedDestinationsRow struct {
	ID           pgtype.UUID
	AccountID    pgtype.UUID
	Kind         string
	DetailsRef   string
...
func (q *Queries) ListUnverifiedDestinations(ctx context.Context, arg ListUnverifiedDestinationsParams) ([]ListUnverifiedDestinationsRow, error) {
	rows, err := q.db.Query(ctx, listUnverifiedDestinations, arg.AfterCreatedAt, arg.AfterID, arg.PageSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListUnverifiedDestinationsRow
	for rows.Next() {
		var i ListUnverifiedDestinationsRow
		if err := rows.Scan(
			&i.ID,
			&i.AccountID,
			&i.Kind,
...
const verifyDestinationAsOperator = `-- name: VerifyDestinationAsOperator :one
...
type VerifyDestinationAsOperatorParams struct {
	VerifiedMethod pgtype.Text
	VerifiedBy     pgtype.UUID
	ID             pgtype.UUID
}
...
func (q *Queries) VerifyDestinationAsOperator(ctx context.Context, arg VerifyDestinationAsOperatorParams) (CashbackPayoutDestination, error) {
	row := q.db.QueryRow(ctx, verifyDestinationAsOperator, arg.VerifiedMethod, arg.VerifiedBy, arg.ID)
