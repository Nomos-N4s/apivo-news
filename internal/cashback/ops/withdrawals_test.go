...
func approve(t *testing.T, approver ops.WithdrawalApprover, id, body string) *httptest.ResponseRecorder {
	...
	ops.NewHandler(discardLogger(), unreachableStore{}, approver, unreachableRefuser{}, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{}, stubAuth{op: anOperator}).ServeHTTP(rec, req)
	return rec
}
...
func TestApprovingNeedsTheOperatorRole(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{}, unreachableRefuser{}, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{},
...
func reject(t *testing.T, refuser ops.WithdrawalRefuser, id, body string) *httptest.ResponseRecorder {
	...
	req.Header.Set("Authorization", "Bearer t")
	rec := httptest.NewRecorder()
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{}, refuser, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{},
...
func settle(t *testing.T, settler ops.WithdrawalSettler, id, body string) *httptest.ResponseRecorder {
	...
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{}, unreachableRefuser{},
		settler, unreachableReconciliation{}, unreachableHeld{}, stubAuth{op: anOperator}).ServeHTTP(rec, req)
	return rec
}
