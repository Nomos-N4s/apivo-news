...
func approve(t *testing.T, approver ops.WithdrawalApprover, id, body string) *httptest.ResponseRecorder {
	...
	ops.NewHandler(discardLogger(), unreachableStore{}, approver, stubAuth{op: anOperator}).ServeHTTP(rec, req)
	return rec
}
...
func TestApprovingNeedsTheOperatorRole(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{},
