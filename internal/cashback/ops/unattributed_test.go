...
func list(t *testing.T, store ops.UnattributedStore, query string) *httptest.ResponseRecorder {
	...
	ops.NewHandler(discardLogger(), store, unreachableApprover{}, unreachableRefuser{}, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{}, unreachableDestinations{}, unreachableAwaiting{}, stubAuth{op: anOperator}).ServeHTTP(rec, req)
	return rec
}
...
func TestTheWrongMethodIsA405WithAnAllowHeader(t *testing.T) {
	...
	req.Header.Set("Authorization", "Bearer t")
	rec := httptest.NewRecorder()
	ops.NewHandler(discardLogger(), unreachableStore{}, unreachableApprover{}, unreachableRefuser{}, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{}, unreachableDestinations{}, unreachableAwaiting{}, stubAuth{op: anOperator}).ServeHTTP(rec, req)
...
func TestEveryRegisteredRouteIsReachable(t *testing.T) {
	...
	h := ops.NewHandler(discardLogger(), &pageStore{}, unreachableApprover{}, unreachableRefuser{}, unreachableSettler{}, unreachableReconciliation{}, unreachableHeld{}, unreachableDestinations{}, unreachableAwaiting{}, stubAuth{op: anOperator})
