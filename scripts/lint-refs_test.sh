...
# while breaking rule 1 (`jules-cb-claude-fix`) is what keeps the second from
...
expect_refused "a blocked token in the middle of a slug" 1 "jules-cb-claude-fix"
expect_refused "a blocked token at the end of a slug" 1 "jules-port-for-claude"
...
expect_refused "a mixed-case token" 1 "jules-Claude-Code-notes"
expect_refused "an underscore as the delimiter" 1 "jules-claude_notes"
expect_refused "a dot as the delimiter" 1 "jules-v2.copilot.draft"
expect_refused "a digit as the delimiter" 1 "jules-gpt-4-summaries"
expect_refused "a versioned assistant" 1 "jules-gpt5-codex-run"
...
expect_refused "one bad name among good ones" 1 \
    "main" "jules-cb-nw-t056" "claude/cb-nw-t057"
expect_refused "two bad names" 2 "claude/a" "jules-ok" "copilot/b"
...
expect_refused "the prefix without a slug" 1 "jules-"
...
expect_accepted "a fully qualified branch" "refs/heads/jules-cb-money"
...
# A tag is judged by rule 1 alone: `v1.2.3` is not `jules-<slug>` and never
...
expect_accepted "the repository's convention" "jules-cb-nw-t056"
expect_accepted "the trunk" "main"
expect_accepted "several ordinary names" "main" "jules-t001-go-dockerfile" "jules-cb-money"

# Delimited matching, tested where it earns its keep.
expect_accepted "a name that merely resembles a blocked token" "jules-claudette-profile"
expect_accepted "another look-alike" "jules-claudia-onboarding"
...
# carry.
expect_accepted "'ai' as a syllable inside ordinary words" "jules-retail-email-domain"
expect_accepted "'agent' meaning a poller, not an assistant" "jules-agent-poller-backoff"
expect_accepted "'bot' meaning automation" "jules-release-bot-guard"
...
expect_accepted "a revert branch GitHub generated" "revert-349-jules-cb-nw-t056"
...
expect_refused_to_judge "an empty name beside a good one" \
    "an empty ref name was given" "jules-ok" ""
...
fixture
subject "Merge pull request #350 from Nomos-N4s/jules-principle-i-refs"
...
fixture
subject "Merge remote-tracking branch 'codex/jules-cb-money' into main"
...
fixture
subject "Merge branch 'jules-cb-money' into claude/cb-nw-t056"
expect_refused "a banned branch on the receiving side of a merge" 1 \
    --from-messages "$BASE..HEAD"
fixture
subject "Merge branch 'jules-cb-money' into main"
...
expect_refused "a remote-tracking merge of a banned branch" 1 \
    --from-messages "$BASE..HEAD"
fixture
subject "Merge remote-tracking branch 'origin/jules-cb-money' into main"
