...
# under internal/*/store, and one per cashback sub-package under
# internal/cashback/*/store) is excluded from the computation: coverage there
...
grep -v -E '/internal/(content|editorial)/store/|/internal/cashback/[^/]+/store/' "$PROFILE" > "$PROFILE.filtered"
