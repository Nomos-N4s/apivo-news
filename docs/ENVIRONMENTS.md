...
| `BRAND_DIR` | unset; `make cashback-demo` points it at the test fixture | **`/etc/apivo/<env>/api.env`**, set to `/etc/apivo/brand` once `/etc/apivo/<env>/brand/brand.json` is written — `provision.sh` creates that directory and `docker-compose.cashback.yml` mounts it into the api read-only, but the key is the operator's because an environment that lists the overlay before a brand exists must still start. Unset, the opt-in answers 503 naming the key and no catalogue import is scheduled; set to a directory holding no complete brand file, the api refuses to start. The procedure is in [RUNBOOK.md](RUNBOOK.md#switching-cashback-on--qa-first) | not shipped in any manifest, deliberately — `deploy/k8s/README.md` says why and how to mount one |
