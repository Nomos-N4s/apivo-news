...
`CASHBACK_ENABLED`, `LEDGER_DRIVER`, `BLNK_URL`, `REDIS_URL`, `CLICK_CONTEXT_HEADER`, `HOLD_*`,
...
| `BRAND_DIR` | unset; `make cashback-demo` points it at the test fixture | **`/etc/apivo/<env>/api.env` and `web.env`**, both set to `/etc/apivo/brand` once `/etc/apivo/<env>/brand/brand.json` is written — `provision.sh` creates that directory, `docker-compose.cashback.yml` mounts it into the api and `docker-compose.yml` into the web, read-only, but the key is the operator's because an environment that lists the overlay before a brand exists must still start. Unset, the opt-in answers 503 naming the key and no catalogue import is scheduled; set to a directory holding no complete brand file, the api refuses to start. The procedure is in [RUNBOOK.md](RUNBOOK.md#switching-cashback-on--qa-first) | not shipped in any manifest, deliberately — `deploy/k8s/README.md` says why and how to mount one |
...
3. **A bigger connection pool.** The scheduler holds one connection per
   ...
   application, so the six jobs a fully configured deployment registers —
   the zero-sum check, the payout settlement sweep, the earnings lifecycle,
   the forward and trailing network sweeps, and the catalogue import — need
   fourteen, and pgx defaults `MaxConns` to `max(4, NumCPU)`. Add
   `pool_max_conns=14` to `DATABASE_URL`. Polling without the catalogue
   import registers five jobs and needs twelve; a cashback deployment doing
   neither registers three and needs eight. Without it the api refuses to
   start, with the numbers in the error, rather than deadlocking its request
   handlers against its jobs under load.
