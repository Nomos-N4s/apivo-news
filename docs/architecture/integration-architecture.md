...
**The finding that shapes the adapter**: query cost tracks the *width of the date window*, not the size of the answer. An empty three-month window still costs about twelve seconds; a year costs 81–102 seconds and once returned nothing at all. Roughly 0.2 s per day of window over a second of overhead. So the thing to ration is days of window per request, not requests per minute.
...
So [conformance_test.go](../../internal/cashback/networks/conformance_test.go) asserts the contract once and runs it against every adapter through a table. A scenario is written against the *port*: it may not name a network's vocabulary, know its pagination, or assume which transactions its recording holds. Everything adapter-specific reaches a scenario through four capabilities on the table entry — open a fresh adapter, name a window with data in it, name a deeplink target, make the network unwell or make it report an unmapped word. A capability an adapter cannot offer is a **skip reported per adapter and per scenario**, never a softened assertion, so a run that exercised one capability cannot read as a run that exercised them all.
...
separated by `chr(31)`, with a null click reference folded to the empty string. A caller-supplied value is discarded. `sha256()` is used as a `pg_catalog` builtin in preference to pgcrypto's `digest()`, because the body is resolved at call time.
...
| **is behind** (reports late) | the forward window would end at now and cover ground nobody has reported | `Limits.ReportingLag` from `network.reporting_lag_minutes` ([0033](../../internal/platform/db/migrations/0033_network_reporting_lag_minutes.up.sql)) stops the window at `now - lag`, so the cursor never passes unreported ground |
| **rate limits us** | `ErrNetworkRateLimited`, distinct from unavailable — it says the pacing was wrong, not that anything is broken | The token bucket paces to `rate_limit_per_minute`; `Retry-After` is honoured as a floor with jitter on top |
...
| Poller, two cursors, window arithmetic, reporting lag | Built and integration-tested ([poller_integration_test.go](../../internal/cashback/networks/poller_integration_test.go), [ingest_integration_test.go](../../internal/cashback/networks/ingest_integration_test.go)) |
